# -*- coding: utf-8 -*-
"""
Created on Fri Nov 29 11:52:21 2019
...
Main.py:
	1. Read the credential json from the json output
	2. Read two Data tables from goolge.drive
	3. 
...
from codes.otherFunction import creating_plot,item_creating,sql_insert,create_conn
...
def main():
    ...
    @sched.scheduled_job(trigger='cron', hour=credentials['scheduler_time']['hour'],minute=credentials['scheduler_time']['minute']) 
    def mainsub():
        ...
        try:
            downloadPath='files/'
            file_downloaded,cnt=downloadfiles(downloadPath,credentials['google_api'],delete_status)
            for i in file_downloaded:
                file_name=i.split('.')[0]
                item_status={'table_name':file_name,'process_date': datetime.date.today(),'process_status':'processed'}
                creating_plot('status_table',item_status,table_creating_command='(process_date date NOT NULL,process_status text NOT NULL,Table_name text NOT NULL)')
                if 'customer' in file_name.lower():
                    new_customer=pd.read_excel(downloadPath+i)
                else:
                    transaction_file=i
            ...
            processing_cl=processing(new_customer=new_customer,ctable_status='uploaded',customer_num_col=customer_num_col, master_list_city=master_list_city, 		master_list_cname=master_list_cname,
                                master_list_address=master_list_address,master_list_province=master_list_province, master_list_shoptype=master_list_shoptype, master_list_contact=master_list_contact, transaction_file_name=downloadPath+transaction_file,
                           qty_col=qty_col, description_col=description_col,date_col=date_col, amount_col=amount_col,booster_conversion=720,starter_conversion=120,cred=cred)
            ...
            transaction_table=processing_cl.processing_history_excepterrorV2()
            ...
            transaction_table['amount_million']=transaction_table['amount']/1000000
            transaction_table=transaction_table[['sku','series','customer_id','customer_name','Date','qty(MC)','qty(pakcs)','amount','amount_million']]
            
            mainlogging.info('loading inserting functions')
            db=inserting_table(transaction_table, customer_table,master_list_city=master_list_city,master_list_province=master_list_province,master_list_shoptype=master_list_shoptype,
                                  master_list_address=master_list_address, master_list_contact=master_list_contact)
            mainlogging.info('mapping customer table with transaction table, preparaing final table')
            df_final,missing_customer_info=db.mapping_customer_information()
            ...
            transaction_problem_table=problem_values(df_final)
        ...
        ############### ERROR ITEM CREATION #####################
            ...
            final_table=pd.read_sql_query("SELECT * from final_table", conn)
           ...
           # raw_ds['amount']=raw_ds.apply(lambda x: x['qty_packs']*15000000 if x['sku']=='STARTER DECK' else  x['qty_packs']*30000000, axis=1)
            ...
            final_table[['system_date','invoice_date']]=final_table[['system_date','invoice_date']].astype(str)
            df_rfm_output=final_table.merge(df_rfm_overall[['customer_id','cluster']],how='left', on='customer_id')
            mainlogging.info('preparaing final json')
          
            final_table=final_table.astype(object).where(final_table.notnull(),None)
            indomart=final_table[final_table['customer_name']=='Indomarco Prismatama, PT']
            non_indomart=final_table[final_table['customer_name']!='Indomarco Prismatama, PT']
            # main_json=final_table.to_dict(orient='records')
            indomart_json=indomart.to_dict(orient='records')
            non_indomart_json=non_indomart.to_dict(orient='records')
            
          
            #main_json=final_table[['sku','series_num','customer_id','invoice_date','year','month','week_of_month','city','province','store_type','qty_packs','amount','customer_name','longtitude','latitude']].to_dict(orient='records')  #amount REMOVED
                 
           # item_main=item_creating(main_json,'Overall_json_output')
           # creating_plot('json_main',item_main,table_creating_command='(Processing_Date text NOT NULL, Type text NOT NULL,  output_json JSONB)')
           # mainlogging.info('preparaing rfm json')
                 
            indomart_main=item_creating(indomart_json,'indomart_json_output')
            creating_plot('json_main',indomart_main,table_creating_command='(Processing_Date text NOT NULL, Type text NOT NULL,  output_json JSONB)')
            mainlogging.info('preparaing rfm json')
      
            nonindomart_main=item_creating(non_indomart_json,'nonindomart_json_output')
            creating_plot('json_main',nonindomart_main,table_creating_command='(Processing_Date text NOT NULL, Type text NOT NULL,  output_json JSONB)')
            mainlogging.info('preparaing rfm json')
            ...
            creating_plot('json_main',item_rfm,table_creating_command='(Processing_Date text NOT NULL, Type text NOT NULL,  output_json JSONB)')
            ...
            creating_plot('json_main',customer_rfm,table_creating_command= '(Processing_Date text NOT NULL, Type text NOT NULL,  output_json JSONB)')
