...
def creating_json_rfm():
    ...
    df_rfm_output=df_rfm_output.round({'Recency':0,'Avg_order_gap':0,'qty_packs':0,'amount':2})
    ...
    from_date = ''
    rfm_json_table = []
    for (description, json_output) in [('rfm filtering by series', json_input_series),\
                                ('rfm filtering by city', json_input_city),\
                                ('rfm filtering by customer name', json_input_cn)]:
     
        rfm_json_table.append ([description, 'rfm', 'NA',\
                                 to_date, from_date, datetime.date.today(), Json(json_output)])
    
    return rfm_json_table                             