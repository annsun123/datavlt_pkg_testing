...
def creating_json_indo(final_table_indo):
    ...
    final_table_indo = final_table_indo[final_table_indo['distribution_center']!='Indomarco Prismatama, PT']
    ...
    final_table_indo['distribution_center']=final_table_indo['distribution_center'].apply(lambda x: x.split('(')[0])
    ...
    topcomp_df = final_table_indo[['distribution_center', 'qty_packs','amount_millions', \
                                  ]].groupby('distribution_center').\
                                  agg({'qty_packs':'sum', 'amount_millions':'sum'}).\
                                  reset_index()
    topcomp_df = topcomp_df.round({'amount_millions':2, 'qty_packs':0})                              
    ...
    topprodct_df = final_table_indo[['product_name', 'qty_packs', 'amount_millions', \
                                  ]].groupby('product_name').\
                                  agg({'qty_packs': 'sum', 'amount_millions': 'sum'}). \
                                  reset_index()
    topprodct_df = topprodct_df.round({'amount_millions':2, 'qty_packs':0})                                
    ...
    geochart_prov_df = final_table_indo[['province', 'qty_packs', 'amount_millions']].groupby('province').\
                                  agg({'qty_packs': 'sum', 'amount_millions': 'sum'}). \
                                  reset_index()
    geochart_prov_df = geochart_prov_df.round({'amount_millions':2, 'qty_packs':0})    
    ...
    geochart_prov_json = {}                                 
    contry_revenue = geochart_prov_df['amount_millions'].sum()
    geochart_prov_df['percentage'] = (geochart_prov_df['amount_millions'] / contry_revenue)*100
    ...
    geochart_citi_df = final_table_indo[['province', 'city', 'qty_packs', 'amount_millions']].groupby(['province', 'city']).\
                                  agg({'qty_packs': 'sum', 'amount_millions': 'sum'}). \
                                  reset_index(level=1)
    geochart_citi_df = geochart_citi_df.round({'amount_millions':2, 'qty_packs':0})                               
    #total revenue for each province
         
              
    table = final_table_indo.groupby('province')['amount_millions'].sum().round(2).to_frame().rename(columns={'amount_millions': 'province_total_amount_million'})
    
    geochart_citi_df['city_color']=0           
   
    for i in table.index:  
        geochart_citi_df.loc[i,  'city_percentage']= (geochart_citi_df.loc[i, 'amount_millions']/table.loc[i,'province_total_amount_million'])*100
    ...
    geochart_citi_df['size_category']=pd.qcut(geochart_citi_df['amount_millions'],4, labels=[i for i in range(4)])
    ...
    table=geochart_citi_df.groupby(['province', 'city'])['amount_millions'].sum().to_frame().rename(columns={'amount_millions': 'city_total_amount_million'}).round(2)
    
    geochart_comp_df = final_table_indo[['province', 'city', 'distribution_center', 'qty_packs', 'amount_millions']].groupby(['province', 'city', 'distribution_center']).\
                                  agg({'qty_packs': 'sum', 'amount_millions': 'sum'}). \
                                  reset_index(level=2)
    geochart_comp_df = geochart_comp_df.round({'amount_millions':2, 'qty_packs':0})                               
    geochart_comp_df['color_category']=pd.qcut(geochart_comp_df['amount_millions'],4, labels=[3,2,1,0])   
    geochart_comp_df['size_category']=pd.qcut(geochart_comp_df['amount_millions'],4, labels=[0,1,2,3])  
    geochart_comp_df['company_percentage']=0
    for i in table.index:
        percentage=[round(value,2) for value in \
        (geochart_comp_df.loc[i, 'amount_millions']/table.loc[i,'city_total_amount_million']*100).tolist()]   
    ...
    company_geo_dic={}
    for i in final_table_indo['distribution_center'].unique():
        company_geo_dic[i] = list(final_table_indo[final_table_indo['distribution_center'] == i].iloc[0][['longtitude', 'latitude']].values)
    geochart_comp_df['longtitude'] = geochart_comp_df['distribution_center'].apply(lambda x: company_geo_dic[x][0])
    geochart_comp_df['latitude'] = geochart_comp_df['distribution_center'].apply(lambda x: company_geo_dic[x][1])    
    ...
    table = final_table_indo[['province',\
                         'city', 'distribution_center', 'qty_packs', \
                         'amount_millions', 'sku', 'series_num']].\
                         groupby(['province','city', 'distribution_center','sku', 'series_num']).\
                          agg({'qty_packs': 'sum', 'amount_millions': 'sum'}).reset_index()
    table = table.round({'amount_millions':2, 'qty_packs':0})                        
    ...
    table = table.merge(table.groupby(['province','city','distribution_center'])['amount_millions'].sum().round(2).reset_index().rename(columns={'amount_millions':'comp_total_rev'}), how='left', on=['province','city','distribution_center'])
    table = table.merge(table.groupby(['province','city','distribution_center','sku'])['amount_millions'].sum().round(2).reset_index().rename(columns={'amount_millions':'sku_total_amount_million'}), how='left', on=['province','city','distribution_center', 'sku'])
    
    table['sku_percentage'] = [round(value,2) for value in (table['sku_total_amount_million'] / table['comp_total_rev']*100).tolist()]
    table['series_percentage'] = [round(value,2) for value in (table['amount_millions'] / table['sku_total_amount_million']*100).tolist()]
    table = table.rename(columns={'amount_millions':'series_total_amount_million'})
    table = table[['province', 'city', 'distribution_center', 'sku', 'series_num', 'qty_packs',\
    ...
    json_df2 = table.groupby(['province','city','distribution_center','sku','backgroundColor',\
    ...
    groupby(['province', 'city', 'distribution_center'])[['sku','sku_total_amount_million', 'sku_percentage','backgroundColor', 'data']].\
    apply(lambda x: x.to_dict(orient='records')).reset_index(name="series_level")
    
    
    doughnut_json_comp={}
    doughnut_ls_comp=[]
    for i in table.distribution_center.unique():
        
        customer_json={}
        customer_json['company_name'] = i
        customer_table = table[table['distribution_center'] == i]
        ...
        customer_json['sku_level']=json_df2[json_df2['distribution_center']==i]['series_level'].iloc[0]
    ...
    table = final_table_indo.groupby(['product_name', 'city'])['amount_millions'].sum().reset_index()
    table = table.round({'amount_millions':2})    
    sku_list=[]
    ...
    for i in sku_list:#table['product_name'].unique():
        ...
        for k in bar_chart_loc['labels']:
            value = table.loc[(table['product_name']== i)& (table['city'] == k)]['amount_millions'].values
            if len(value) > 0:
    ...
    # sort by company
    table2 = final_table_indo.groupby(['product_name', 'distribution_center'])['amount_millions'].sum().reset_index()
    table2 = table2.round({'amount_millions':2}) 
    bar_chart_com={}
    bar_chart_com['labels'] = list(final_table_indo['distribution_center'].unique())
    ...
    for i in sku_list:#table['product_name'].unique():
        ...
        for k in bar_chart_com['labels']:
            value = table2.loc[(table2['product_name']== i)& (table2['distribution_center'] == k)]['amount_millions'].values
            if len(value) > 0:
                value_list.append(round(value[0],3))
            else:
                value_list.append(0)
    ...
    table3 = final_table_indo.groupby(['product_name', 'city', 'distribution_center'])['amount_millions'].sum().reset_index()
    table3 = table3.round({'amount_millions':2})  
    bar_comps_city={}
    ...
    for city in final_table_indo['city'].unique():
        ...
        bar_comps = {}
        bar_comps['labels'] = list(final_table_indo[final_table_indo['city'] == city]['distribution_center'].unique())
        sku_value_list=[]
        for i in sku_list:
            value_dic ={}
            value_dic['label'] = i
    ...
    groupby(['distribution_center', 'year', 'month', 'product_name',\
             'week_of_month'])['amount_millions'].sum().reset_index()#level=-1)
    table = table.round({'amount_millions':2})           
    ...
    final_chart_json={}
    final_chart=[]
    for dc in table['distribution_center'].unique():
        dc_dic={}
       
        dc_table = table[table['distribution_center']==dc].reset_index(drop=True)
        dc_dic['distribution_center']=dc
        ...
        spar_matrix=csr_matrix((dc_table['amount_millions'], (dc_table['index'], dc_table['beg_date_cat'])))
