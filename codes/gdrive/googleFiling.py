...
def downloadfiles(downloadPath,google_api,delete_status):
    ...
    gauth.LoadCredentialsFile('./codes/gdrive/credentials.json') # client_secrets.json need to be in the same directory as the script
   ...
   # fileList = drive.ListFile({'q': "'19nFd8wCUMsjVqrbEQpfdBaAjj2ja-31g' in parents and trashed=false"}).GetList()
    ...
    for file in fileList:
        if 'sheet' in file.metadata['mimeType'] or 'ms-excel' in file.metadata['mimeType']: #file['title']!='Archive':
            cnt+=1

            dowfilelogger.info('Downloading file %s from Google Drive' % file['title']) # 'hello.png'
