...
class processing:
    ...
    def __init__(self, customer_indomaret, customer_nonindomaret,
                 ctable_status, customer_num_col, master_list_city, 
                 master_list_cname, master_list_address, master_list_province,
                 master_list_shoptype, master_list_contact, 
                 transaction_nonindo_file,qty_col, description_col,
                 date_col, amount_col, booster_conversion,
                 starter_conversion, cred):
        ...
        if(rst):
            ...
            if self.ctable_status == 'not_uploaded':
                ...
                try:
                    ...
                    for i in [self.master_list_city, self.master_list_cname, 
                              self.master_list_address, self.master_list_contact, 
                              self.master_list_shoptype, self.master_list_province, 'indo_info']:
...
# electing all rows in the customer_indomrate table, and select rows with both Address and City information for indomarate as df_customer_api. [city==np.nan and address==np.nan are reported as error]
                    ...
                    df_customer_api=df_customer[
                        ~(pd.isnull(
                            df_customer[self.master_list_address]) | pd.isnull(df_customer[self.master_list_city])
                         ) &
...
# call api for df_customer_api where longtitude is null. [if the longtitue is not null which means it already has longtitude information, no need to call.]
                    ...
                    table, error_address = google_geocode1(
                        ...
                        'full_address', 'customer_id', self.cred
                    )
...
# the output table does have all the longtitude and longtitude information. To mapping the table information to the df_customer_api table
                    ...
                    df_customer_api.loc[df_customer_api['longtitude'].isnull(),
                                        ['formal_address',
                                         'longtitude',
...
# These information will be reported and adding to the main table as non-process version[no longittude and latitude values]
                    ...
                    final_customer_table = df_customer_api.merge(
                        problematic_table, how='outer'
                    ).merge(table, how='outer')
                    ...
                    final_customer_table, problematic_table = [
                        x.drop('Unnamed: 0',1)  if 'Unnamed: 0' in x 
                        else x for x in [final_customer_table,problematic_table]
                    ] 

                    final_customer_table = final_customer_table[['customer_id', 
                                                               self.master_list_cname, self.master_list_address, 
                                                               'formal_address', self.master_list_city, 
                                                               self.master_list_province, self.master_list_shoptype, 
                                                               self.master_list_contact, 'longtitude',
                                                               'latitude', 'error_type', 'indo_info']]
            ...
            if self.ctable_status == 'not_uploaded':
                ...
                try:
                    ...
                    final_customer_table = final_customer_table[['customer_id',
                                                                 self.master_list_cname, self.master_list_address,
                                                                 'Formatted Address', self.master_list_city,
                                                                 self.master_list_province, self.master_list_shoptype,
                                                                 self.master_list_contact, 'Longitude','Latitude',
                                                                 'error_type', 'indo_info']]
                    
                    final_customer_table.columns = ['customer_id', self.master_list_cname, 
                                                    self.master_list_address, 'formal_address',   
                                                    self.master_list_city, self.master_list_province,
                                                    self.master_list_shoptype, self.master_list_contact,
                                                    'longtitude','latitude', 'error_type', 'indo_info']
                    final_customer_table['updated_date'] = datetime.date.today()
...
# df_final_test.append(day_fill_test1[day_fill_test1.columns[index_list2[i:i+3]]])
            ...
            for i in range(0, len(index_list2), 3):
                ...
                data.columns = ['Date','customer_id', 'customer_name',
                              'sku_type', 'qty(MC)', 'amount']
...
# setting colnames - invoie date
            ...
            df_new.rename(
                columns={invoice_date[invoice_date==True].index[0]: 'Invoice Date'},
                inplace=True
            )
