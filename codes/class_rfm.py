...
def createRFM_dataset(df):
    df_rfm = df.groupby(['customer_name',
                         'customer_id', 'city' ]).agg(
        {'invoice_date': 'max', 'Avg_order_gap': 'mean',
         'qty_packs': 'count', 'amount': 'sum'}
    ).reset_index()
    df_rfm['Recency'] = [pd.to_datetime(
        df_rfm.loc[:, 'invoice_date']
    ).max() - x for x in pd.to_datetime(df_rfm.loc[:, 'invoice_date'])]
    ...
    df_rfm['F_score'] = pd.cut(df_rfm['Frequency'], 4, labels=['1', '2', '3', '4'], 
                               duplicates = 'drop').astype(int)
...
def clustering(df, str_cols, opt_clust):
    ...
    rfmlogger.info('silhouette score', 
                   silhouette_score(
                       scaler.fit_transform(df[str_cols]),
                       pred_clusters, random_state=42)
                  )
...
def defineCluster(df, str_cluter_def):
    ...
    for i in range(4):
        df.loc[df['cluster'] == list(
            dict_clust_def_overall.keys()
        )[i],'cluster'] = list(dict_clust_def_overall.values())[i]
