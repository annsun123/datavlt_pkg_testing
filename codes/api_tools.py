...
def google_geocode(address_list, cred):
    ...
    LIMIT = 25000  #setting the daily limit call
    unused_limit = LIMIT
    ...
    for idx, address in enumerate(address_list):
        answer = {  #create dictionary for recording info.
            'address': address,
        ...
        params = {'address': address, 'key': cred}  #cred_key is recorded in the overall credential files. in json_credential folder
        response = requests.get(url, params=params)
        if response.status_code == 400: #if address and key are ok, continue to call the address
        ...
        # this is recording #calling, preventing it exceed calling limit. When calling reach limit. need to wait for another 24 hours for another 20000 calls 
        unused_limit -= 1
        if unused_limit < 1:
