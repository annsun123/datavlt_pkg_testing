...
class inserting_table:
    def __init__(self, transaction_nonindo, transaction_indo,
		 final_customer_table, master_list_city, master_list_province,
		 master_list_shoptype, master_list_address, master_list_contact):
...
#customer table by using the methods of left merging 
		    ...
		    customer_non_indo[['customer_id', 'city',
				       'province', 'shop_type',
				       'customer_address',
                                       'latitude', 'longtitude']],
            ...
            non_customer_info = [
		    x for x in final_non_indo['customer_id'].unique() 
	    ...
	    self.transaction_indo['longtitude'] = np.nan
            ...
            for x in self.transaction_indo['Distribution Center'].unique():
                if  x in customer_indo['customer_name'].unique():
                    ...
                    self.transaction_indo.loc[
			    self.transaction_indo['Distribution Center']==x,
			    [self.master_list_city, self.master_list_province,
			     self.master_list_shoptype, 'latitude',
                            'longtitude']] = customer_indo[
			    customer_indo['customer_name']==x
		    ][['city', 'province', 'shop_type',
		       'latitude', 'longtitude']].iloc[-1].values
                else:
            ...
            df_transaction['week_of_month'] = df_transaction['Invoice Date'].apply(
		    lambda x: np.nan 
		    if pd.isnull(x)
		    else int(math.ceil((x.day + x.replace(day = 1).weekday()) / 7.0))
	    )
